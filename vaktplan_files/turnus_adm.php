...
for($ja=0; $ja< count($groupid); $ja++){
    ...
    if(is_array($Turnus))$numTours = count ($Turnus);
    ...
    //populer turnuser med data fra database, etter starttid på turnus
    if ($numTours == 1 && $Turnus[0][1] > date("Y-m-d", $TimeVar) ){ //kun en tur, Startdato i fremtiden
        $TurnusPlanNeste = hentTurnus($Turnus[0][0]);
        $NextTourID = $Turnus[0];
    }
    if ($numTours == 1 && $Turnus[0][1] <= date("Y-m-d", $TimeVar)){ //kun en tur, nåværende
        $TurnusPlan = hentTurnus($Turnus[0][0]);
        $CurrentTourID = $Turnus[0];
    }
    if ($numTours == 2 && $Turnus[0][1] > date("Y-m-d", $TimeVar) ){ // To turnuser, nåværende og fremtidig
        $TurnusPlan = hentTurnus($Turnus[1][0]);
        $TurnusPlanNeste = hentTurnus($Turnus[0][0]); 
        $CurrentTourID = $Turnus[1];
        $NextTourID = $Turnus[0];
    }
    if ($numTours == 2 && $Turnus[0][1] <= date("Y-m-d",$TimeVar)){ // To turnuser, nåværende og forrige
    ...
    //error_log (print_r($TurnusPlan, true));
...
//	$ansatte[$t][0]
	...
	for($counter = 0; $counter < count($ansatte); $counter++){
	    if(!harTur($ansatte[$counter][0],$groupid[$ja], date("Y-m-d", $TimeVar)) 
	           || harSnartTur($ansatte[$counter][0],$groupid[$ja], date("Y-m-d", $TimeVar))
			...
			<div class="TurnusViewer UnassignedPersonell " ID="UnassignedPersonell">
					...
					<div class="Unassigned">
						...
						if (is_array($utentur2)){
    						...
    						?>
    							<div class="dragdropTurAssign LTail floatDown setStartDateAnsattTour" id="<?php echo $utentur2[$jp][0]; ?>">
    								<?php 
                					...
                					{
                					    //error_log("Blippa: " . print_r($CurrentTourID, true));
                					    
                					    $chckFree = isTurFree($nqi, $CurrentTourID[2]); //eksisterende tur, noen har hatt den frem til en dato
                					    if($chckFree[0] != ''){
                					        //$AvailStart = time();
                					        $AvailStart = strtotime($chckFree[0] . "+1 Day");
                					        $AvailTour[$blupp] = array("TourID" =>$nqi, "AvailDate" => Date("Y-m-d",$AvailStart ));
                					        $blupp++;
                					    }
                					    if (!is_array($chckFree)){ //ny tur, aldri besatt
	...
	//			echo $w;
		...
		<div class="TurnusViewer CurrentTurnus">
			...
			<div class="Assign AssignedPersonell floatLeft" ID="AssignedPersonell">
					<div class = " Assigned floatDown LTail" id="<?php echo  $CurrentTourID[2] . '-L'; ?>">
						<?php $navn = finnAnsattNavn($leder); ?>
						<div class="dragdropTurAssign setEndDateAnsattTour" id="<?php echo $lederTurID; ?>">
						<?php 
							echo $navn[0];
							?>
						</div>
				...
				?>
					<div class = "Assigned floatDown LTail" id="<?php echo  $CurrentTourID[2] . '-' .$x; ?>">
						<?php if ($AnsID[0] != ''){	?>
						<div class="dragdropTurAssign setEndDateAnsattTour" id="<?php echo $AnsID[0]; ?>">
						<?php 
							echo $navn[0] . $endDate;
							?>
						</div>
	...
	//			echo $w;
		...
		<div class="setEndDateTourDialog" title="Sluttdato">
			<form action="vaktplan_files/Turnus_WriteDB_TurnusAdm.php" method="post" class="editForm"  autocomplete="off">
					...
					<br />Tøm, eller la være tomt, for ingen sluttdato<br />			
					<input type="checkbox" name="endTildeling" id ="endTildeling"  checked /><label class="dialogTXT"> Avslutt tur-tildelinger også </label></br>
					
					<input type="hidden" ID="TourID">
					<input type="hidden" ID="TourGroupreset" value="<?php echo $groupid[$ja] ?>">
			</form>
		</div>
